...
public class Fraction implements Comparable<Fraction>{
	...
	/**
	 * Display fractions as they are stored. It's the same as {@link #DISPLAY_AS_FRACTION}.
	 */
	...
	/**
	 * Creates a new fraction using numerator and denominator.
	...
	public Fraction(long numerator, long denominator) {
		if(denominator == 0) throw new IllegalArgumentException("Denominator can not be zero.");
		this.numerator = numerator;
		this.denominator = denominator;
	}
	
	/**
	 * Creates new fraction using a floating point number.
	 * It stores the given floating point number as the numerator and 1 as denominator.
	 * Then keeps multiplying by 10 until there are no more decimal places.
	 * 
	 * Example:
	 * n = 3.14159265, would give us
	 * 
	 * nominator = 314159265
	...
	public Fraction(double n) {
		...
		this.numerator = numerDummy.longValueExact();
	...
	/**
	 * Returns the floating point value of this fraction.
	...
	/**
	 * Reduces this fraction object.
	 * Uses the {@link Fraction#gcd(long, long)} Method
	 * and divides numerator and denominator by the gdc.
	 ...
	 * @param _displayType
	 * <p>
	 * <li>{@link #DISPLAY_AS_FRACTION} - Shows probability as fraction</li>
	 * <li>{@link #DISPLAY_AS_FLOAT} - Shows probability as float</li>
	 * <li>{@link #DISPLAY_AS_SIMPLIFIED_FRACTION} - Shows probability as simplified fraction</li>
	 */
	...
	public String toString() {
		switch(displayType){
			case DISPLAY_AS_FRACTION: return this.toFractionString();
			case DISPLAY_AS_FLOAT: return "" + this.toFloatingPoint();
			case DISPLAY_AS_SIMPLIFIED_FRACTION: return "" + this.simplify().toFractionString();
			default: return this.toFractionString();
		}
	}
	...
	public static Fraction division(Fraction a, Fraction b){
		if(b.getNumerator() == 0) throw new IllegalArgumentException("You can not devide by zero.");
	...
	public static void main(String[] args) {
		...
		for(int i=0; i<n; i++) {
			long num = 2+(int)(1000*Math.random());
			long den = 2+(int)(1000*Math.random());
			fracs[i] = new Fraction(num,den);
			ds[i] = (1.0*num)/den;
		}
		...
		for(int i=1; i<n; i++) {
			result = multiplication(fracs[i],fracs[i-1]);
			result = division(fracs[i],result);
		}
